...
public class HttpWebClient extends WebClientBase {
	...
	protected static DefaultHttpClient newDefaultHttpClient() {
		...
		final DefaultHttpClient ret = new DefaultHttpClient(new ThreadSafeClientConnManager(httpParams, registry),
				httpParams);
		return ret;
	}
	...
	protected HttpException newHttpException(final HttpUriRequest request, final HttpResponse response) {
		...
		final String message = new StringBuilder().append(statusCode).append(' ').append(statusLine.getReasonPhrase())
				.append(" at ").append(request.getURI()).toString();
	 ...
	 * @throws HttpException
	 */
	public HttpResponse executeHttp(final HttpUriRequest request, final int beginExpectedCode, final int endExpectedCode)
			throws ClientProtocolException, IOException, HttpException {
		final HttpResponse response = httpClient.execute(request);
	 ...
	 * @throws HttpException
	 */
	public HttpResponse executeHttp(final HttpUriRequest request) throws ClientProtocolException, IOException,
			HttpException {
